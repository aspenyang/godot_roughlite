...
#  - Provide helpers for checkpoints, level lifecycle, persistence
#  - Player state no longer stored inside checkpoint (simplified)
#
...
func load_slot(slot: int) -> void:
	...
	if save_data.is_empty():
		save_data = SaveManager.get_default_data(slot, levels_total)
...
func get_player_state_from_checkpoint() -> Dictionary:
	return {} # Deprecated: no player state stored

func set_checkpoint(scene_path: String, level_id: int) -> void:
	if active_slot < 0:
		return
	save_data = SaveManager.set_checkpoint(active_slot, scene_path, level_id, levels_total)
	is_resuming_from_checkpoint = false
...
func update_checkpoint_player_state(_player_state: Dictionary) -> void:
	pass # No-op
...
func begin_round_if_needed() -> void:
	# Backwards compatibility wrapper -> level terminology
	if active_slot >= 0:
		save_data = SaveManager.begin_level_if_needed(active_slot)

func complete_round_success(final_round: bool) -> void:
	if active_slot >= 0:
		save_data = SaveManager.complete_level_success(active_slot, final_round)

func fail_round(final_round: bool, record_time: bool = false) -> void:
	if active_slot >= 0:
		save_data = SaveManager.fail_level(active_slot, final_round, record_time)
...
func get_rounds_completed() -> int:
	return int(save_data.get("levels_completed", 0))

func get_rounds_total() -> int:
	return int(save_data.get("levels_total", levels_total))

func get_round_times() -> Array:
	return save_data.get("level_times", [])
...
func get_total_run_time() -> float:
	...
	for t in get_round_times():
		if typeof(t) in [TYPE_FLOAT, TYPE_INT]:
			total += float(t)
	var current_start := float(save_data.get("current_level_start_time", 0.0))
...
func prepare_level_entry(scene_path: String, level_id: int) -> void:
	...
	set_checkpoint(scene_path, level_id)
...
func debug_print_state() -> void:
	...
	print("levels_completed:", get_rounds_completed(), "/", get_rounds_total())

func maze_used() -> bool:
	return bool(save_data.get("maze_used", false))
func reward_used() -> bool:
	return bool(save_data.get("reward_used", false))
